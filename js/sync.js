...
async function doSyncRequest(params) {
  // Apps Script only reliably accepts GET requests from external origins.
  // All data is sent via URL query parameters.
  // For large push payloads we use LZW-style base64 compression via btoa.
  const action = params.action || '';
  let url = DB.syncUrl + '?action=' + encodeURIComponent(action);

  if (action === 'push' && params.data) {
    // Compress data: JSON → base64 to keep URL shorter
    const json = typeof params.data === 'string'
      ? params.data
      : JSON.stringify(params.data);
    // Split into chunks if too large (URL limit ~7000 chars safe)
    const encoded = btoa(unescape(encodeURIComponent(json)));
    if (url.length + encoded.length < 7000) {
      url += '&data=' + encodeURIComponent(encoded) + '&enc=b64';
    } else {
      // Fallback: send without compression, truncate non-critical fields
      const slim = slimPayload(params.data);
      url += '&data=' + encodeURIComponent(JSON.stringify(slim));
    }
  }
...
// Slim down payload for large datasets — keep only recent data
function slimPayload(data) {
  ...
  return {
    ...data,
    expenses: (data.expenses || []).filter(e => e.date >= cutStr),
    incomes:  (data.incomes  || []).filter(i => i.date >= cutStr),
  };
}
